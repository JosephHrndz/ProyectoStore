...
namespace ProyectoStore.DAL
{
    public class UsuarioDAL
    {
        ...
        private static void EncriptarMD5(Usuario pUsuario)
        {
            ...
            {
                var result = md5.ComputeHash(Encoding.UTF8.GetBytes(pUsuario.Contraseña));
                var strencriptar = "";
                for (int i = 0; i < result.Length; i++)
                    strencriptar += result[i].ToString("x2").ToLower();
                pUsuario.Contraseña = strencriptar;
            }
        ...
        internal static IQueryable<Usuario> QuerySelect(IQueryable<Usuario> pQuery, Usuario pUsuario)
        {
            ...
            if (!string.IsNullOrWhiteSpace(pUsuario.Usuarios))
                pQuery = pQuery.Where(s => s.Usuarios.Equals(pUsuario.Usuarios));
        ...
        public static async Task<Usuario> LoginAsync(Usuario pUsuario)
        {
            ...
            {
                EncriptarMD5(pUsuario);
                usuario = await dbContexto.Usuario.FirstOrDefaultAsync(s => s. Usuarios == pUsuario.Usuarios &&
                s.Usuarios == pUsuario.Usuarios && s.Estatus == (byte)Estatus_Usuario.ACTIVO);
            }
            return usuario;
        }
        ...
        {
            int result = 0;
            var usuarioAnt = new Usuario { Contraseña = pPasswordAnterior };
            ...
            using (var dbContexto = new DBContexto())
            {
                ...
                if (usuarioAnt.Contraseña == usuario.Contraseña.Trim())
                {
                    ...
                    throw new Exception("la contraselña actual es incorrecto");
            }
            return result;
        }
    }
